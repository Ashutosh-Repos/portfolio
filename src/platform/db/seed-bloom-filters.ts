...
export const BLOOM_FILTER_ARTICLE = {
  ...
  contentMarkdown: `A Bloom filter is a probabilistic data structure that answers a very specific question - have I seen this thing before? - while using almost no memory.
...
- **MurmurHash3**: Widely considered the best general-purpose non-cryptographic hash. It provides excellent distribution, handles all input sizes well, and is very fast. MurmurHash3 supports both 32-bit and 128-bit outputs.
...
When evaluating Bloom filter implementations, several metrics matter and most of these benchmarks can be found on Arpit Bhayani's repository [abloom](https://github.com/arpitbbhayani/abloom).
...
- Recommendation systems filtering previously shown items.
- Web crawlers tracking visited URLs.
...
### Database Acceleration: PostgreSQL Bloom Indexes
...
Bloom filters, invented by Burton Howard Bloom in 1970, remain one of the most widely deployed probabilistic data structures. Their simplicity, efficiency, and well-understood mathematical properties put them at the core of modern systems design - from database internals to distributed systems to network security.
