...
export class LeetCodeAdapter
  implements ExternalProvider<RawLeetCodePayload, LeetCodeNormalizedData>
{
  ...
  normalize(raw: RawLeetCodePayload): LeetCodeNormalizedData {
    ...
    const getCount = (arr: LeetCodeDifficultyCount[], diff: string) => {
      return (
        arr.find((item) => item.difficulty.toLowerCase() === diff.toLowerCase())
          ?.count || 0
      );
    };
    ...
    const totalSubmissionsArray =
      user?.submitStatsGlobal?.totalSubmissionNum || [];
    const totalSubmissions =
      totalSubmissionsArray.find((item) => item.difficulty === 'All')
        ?.submissions || 0;
    const acceptedSubmissions =
      solvedArray.find((item) => item.difficulty === 'All')?.submissions || 0;
    ...
    if (tagCounts) {
      (['fundamental', 'intermediate', 'advanced'] as const).forEach(
        (level) => {
