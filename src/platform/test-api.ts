...
async function verifyPlatform() {
  ...
  console.log(`✅ Education (${education.length} entries):`, education.map(e => e.institution));

  // 3. Experience
  const experiences = await experienceService.getExperiences();
  console.log(`✅ Experiences (${experiences.length} entries):`, experiences.map(e => `${e.company} (${e.role})`));

  // 4. Projects
  const projects = await projectService.getProjects();
  console.log(`✅ Projects (${projects.length} entries):`, projects.map(p => p.title));
  ...
  console.log(`✅ Writings (${blogs.items.length} blogs, ${essays.items.length} essays, ${papers.items.length} research papers)`);

  // 6. Skills
  const skills = await skillsService.getSkills({ featuredOnly: true });
  console.log(`✅ Featured Skills (${skills.length} skills):`, skills.map(s => s.name).join(', '));
  ...
  console.log(`✅ Hobbies (${movies.length} movies, ${series.length} series):`, [
    ...movies.map(m => `${m.title} (${m.myRating}★)`),
    ...series.map(s => `${s.title} (${s.myRating}★)`),
  ].join(', '));

  // 8. Snapshots
  const ghSnapshot = await syncService.getSnapshot<GitHubNormalizedData>('github_overview');
  console.log(`✅ GitHub Snapshot: ${ghSnapshot.data ? `${ghSnapshot.data.user.username} (${ghSnapshot.data.impact.stars} stars, ${ghSnapshot.data.languages.length} languages)` : 'None'}`);

  const lcSnapshot = await syncService.getSnapshot<LeetCodeNormalizedData>('leetcode_overview');
  console.log(`✅ LeetCode Snapshot: ${lcSnapshot.data ? `${lcSnapshot.data.user.username} (${lcSnapshot.data.solved.all} solved, rank ${lcSnapshot.data.user.ranking})` : 'None'}`);
