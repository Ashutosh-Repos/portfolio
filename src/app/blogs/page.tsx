...
export default async function BlogsPage() {
  const result = await blogService.getBlogs({ limit: 100 }).catch(() => ({ items: [] }));
        ...
        <div className="py-1 px-0.5 flow-root">
          <div className="flex items-baseline justify-between gap-4">
            <h1 className="text-foreground">
              <span className="font-light text-2xl md:text-3xl">Technical</span>{' '}
              <span className="text-foreground/50 text-xl">blog</span>{' '}
              <span className="italic text-2xl font-light">posts</span>
            </h1>
