...
export default async function WritingsPage() {
  const result = await writingService.getWritings({ type: 'essay', limit: 100 }).catch(() => ({ items: [] }));
        ...
        <div className="py-1 px-0.5 flow-root">
          <div className="flex items-baseline justify-between gap-4">
            <h1 className="text-foreground">
              <span className="font-light text-2xl md:text-3xl">Personal</span>{' '}
              <span className="text-foreground/50 text-xl">thoughts</span>{' '}
              <span className="italic text-2xl font-light">& essays</span>
            </h1>
          ...
          <p className="pt-3 text-sm sm:text-base text-foreground/80 leading-relaxed max-w-2xl">
            I write whenever inspiration strikes, which means I&apos;m pretty irregular about it. Reflections on engineering philosophy, intentional craft, and ambition.
          </p>
