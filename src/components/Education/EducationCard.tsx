...
export function EducationCard({ education }: EducationCardProps) {
  ...
  return (
    <div className="p-6 rounded-2xl border border-black/[0.08] dark:border-white/[0.08] bg-foreground/[0.02] flex flex-col gap-4 transition-all">
      {/* Institution Header */}
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-foreground/[0.05] text-foreground/80">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {previewPhotos.map((photo, idx) => (
              ...
              >
                <Image
                  src={photo.publicUrl}
                  alt={photo.altText || photo.caption || education.institution}
                  fill
        ...
        >
          ...
          >
            ...
            <div className="w-full flex items-center justify-between text-white font-mono text-xs px-2">
              <span>
                {activePhotoIndex + 1} / {photos.length} &bull; {education.institution}
              </span>
              <button
                type="button"
                onClick={() => setActivePhotoIndex(null)}
